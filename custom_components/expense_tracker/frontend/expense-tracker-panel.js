...
class ExpenseTrackerPanel extends LitElement {
  ...
  async _loadExpenses() {
    const res = await this._ws("expense_tracker/expenses/list", {
      month: this._currentMonth,
    });
  ...
  async _deleteExpense(id) {
    const res = await this._ws("expense_tracker/expenses/delete", {
      expense_id: id,
    });
    if (res) {
  ...
  async _saveBudget(category, amount) {
    const res = await this._ws("expense_tracker/budgets/set", {
      category,
      amount: Number(amount),
    });
    if (res) {
  ...
  async _settleDebt(settlement) {
    ...
    const confirmed = confirm(
      `${this._t("set_record_prompt")} ${settlement.from_name} ${this._t("set_paid")} ${settlement.to_name} ${sym}${settlement.amount}?`
    );
    if (!confirmed) return;
  ...
  _renderNotification() {
    return html`
      <div class="notification ${this._notification.type}">
        <ha-icon
          icon=${this._notification.type === "error"
            ? "mdi:alert-circle"
            : "mdi:check-circle"}
        ></ha-icon>
          ...
          <div class="form-row">
            <div class="form-group flex-1">
              ...
              <input
                ...
                @input=${(e) => {
                  this._formData = { ...this._formData, date: e.target.value };
                }}
                class="text-input"
              />
            </div>
