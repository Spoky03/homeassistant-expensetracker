...
import { getCategoryColor, getCategoryIcon, isDefaultCategory, } from "./shared/categories.js";
import { formatCurrency, getCurrentMonth, getMonthName, getPreviousMonth, getNextMonth, } from "./shared/format.js";
...
class ExpenseTrackerPanel extends LitElement {
    ...
    async _loadSummary() {
        const res = await this._ws("expense_tracker/summary", {
    ...
    async _saveBudget(category, amount) {
        const res = await this._ws("expense_tracker/budgets/set", { category, amount: Number(amount) });
        if (res) {
    ...
    async _settleDebt(settlement) {
        const sym = this._config.currency_symbol ?? "€";
        const confirmed = confirm(`${this._t("set_record_prompt")} ${settlement.from_name} ${this._t("set_paid")} ${settlement.to_name} ${sym}${settlement.amount}?`);
        if (!confirmed)
            return;
        this._loading = true;
        const res = await this._ws("expense_tracker/expenses/add", {
        ...
        <div class="view-header">
          ...
          <div class="month-nav">
            ...
            <span class="month-label"
              >${getMonthName(this._currentMonth, this.hass?.locale?.language || this.hass?.language)}</span
            >
          ...
          <div class="glass-card recent-card">
            ...
            ? html `
                  <div class="recent-list">
                    ${this._expenses.slice(0, 8).map((e) => html `
    ...
    _renderDonutChart(categories, total, sym) {
        ...
        const conicSegments = categories
            ...
            .join(", ");
        const donutStyle = conicSegments.length > 0
    ...
    _renderBalancesAndSettlements(sym) {
        if (!this._summary)
            return "";
            ...
            : html `
              <div class="glass-card empty-card">
                ...
                <p>
                  ...
                  ${getMonthName(this._currentMonth, this.hass?.locale?.language || this.hass?.language)}
                </p>
                <button
                  class="primary-btn"
                  @click=${() => {
