...
export const getPreviousMonth = (m: string): string => shiftMonth(m, -1);
export const getNextMonth = (m: string): string => shiftMonth(m, +1);

export const getMonthName = (monthStr: string, locale: string = "default"): string => {
  const [y, m] = monthStr.split("-");
